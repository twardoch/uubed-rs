// this_file: crates/uubed-python/src/lib.rs
//! Python bindings for uubed-core using PyO3.

// PyO3 0.22 macro-generated conversions trigger newer Clippy.
...
fn q64_encode_native(data: &[u8]) -> String {
    codec::encoders::q64_encode(data)
}
...
fn q64_decode_native(encoded: &str) -> PyResult<Vec<u8>> {
    codec::encoders::q64_decode(encoded).map_err(|e| PyValueError::new_err(e.to_string()))
}
...
fn q64_encode_buffer_native(py: Python<'_>, data: PyBuffer<u8>) -> PyResult<Bound<'_, PyBytes>> {
    ...
    let encoded = codec::encoders::q64_encode(input_slice);
...
fn q64_encode_batch_native(
    ...
    let mut buffer_pool: Option<Vec<u8>> = if reuse_buffers {
        Some(Vec::new())
    } else {
        None
    };
    ...
    for data_buffer in embeddings {
        ...
        if let Some(ref mut pool_buffer) = buffer_pool {
            ...
            codec::encoders::q64_encode_to_buffer(input_slice, &mut pool_buffer[..required_len])
...
impl Q64StreamEncoder {
    ...
    fn encode_chunk<'a>(
        ...
        codec::encoders::q64_encode_to_buffer(input_slice, &mut self.buffer[..required_len])
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
...
fn q64_encode_inplace_native(
    ...
    let output_slice = match output_buffer.as_mut_slice(py) {
        ...
        None => {
            return Err(PyValueError::new_err(
                "Failed to access output buffer as mutable",
            ))
        }
    };
    ...
    let encoded_str = codec::encoders::q64_encode(input_slice);
...
fn simhash_q64_native(embedding: &[u8], planes: usize) -> String {
    codec::encoders::simhash_q64(embedding, planes)
}
...
fn top_k_q64_native(embedding: &[u8], k: usize) -> String {
    codec::encoders::top_k_q64(embedding, k)
}
...
fn top_k_q64_optimized_native(embedding: &[u8], k: usize) -> String {
    codec::encoders::top_k_q64_optimized(embedding, k)
}

/// Generate a Z-order (Morton code) Q64 encoding for an embedding
#[pyfunction]
#[pyo3(signature = (embedding))]
fn z_order_q64_native(embedding: &[u8]) -> String {
    codec::encoders::z_order_q64(embedding)
}
...
fn mq64_encode_native(data: &[u8], levels: Option<Vec<usize>>) -> PyResult<String> {
    let s = match levels {
        Some(lvls) => codec::encoders::mq64_encode_with_levels(data, &lvls),
        None => codec::encoders::mq64_encode(data),
    };
    Ok(s)
}
...
fn mq64_decode_native(encoded: &str) -> PyResult<Vec<u8>> {
    codec::encoders::mq64_decode(encoded).map_err(|e| PyValueError::new_err(e.to_string()))
}
...
fn simhash_to_buffer_native(
    ...
    codec::encoders::simhash_to_buffer(input_slice, planes, output_slice)
        .map_err(|e| PyValueError::new_err(e.to_string()))
}
...
fn top_k_to_buffer_native(
    ...
    // Encode directly to output buffer
    codec::encoders::top_k_to_buffer(input_slice, k, output_slice)
        .map_err(|e| PyValueError::new_err(e.to_string()))
}
...
fn z_order_to_buffer_native(
    ...
    // Encode directly to output buffer
    codec::encoders::z_order_to_buffer(input_slice, output_slice)
        .map_err(|e| PyValueError::new_err(e.to_string()))
}
...
fn parallel_q64_encode_native(
    ...
    // Encode in parallel
    Ok(codec::parallel::parallel_q64_encode(
        &embedding_refs,
        num_threads,
    ))
}
...
fn parallel_simhash_encode_native(
    ...
    // Encode in parallel
    Ok(codec::parallel::parallel_simhash_encode(
        &embedding_refs,
        planes,
        num_threads,
    ))
...
fn parallel_topk_encode_native(
    ...
    // Encode in parallel
    Ok(codec::parallel::parallel_topk_encode(
        &embedding_refs,
        k,
        num_threads,
    ))
