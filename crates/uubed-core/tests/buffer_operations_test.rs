use uubed_core::encoders::{
    q64_encode, q64_encode_to_buffer, simhash_q64, simhash_to_buffer, top_k_q64_optimized,
    top_k_to_buffer, z_order_to_buffer,
};
...
fn test_simhash_to_buffer_success() {
    ...
    let planes: usize = 32;
    let expected_bytes = planes.div_ceil(8); // 4 bytes for 32 planes
...
fn test_simhash_to_buffer_different_plane_counts() {
    ...
    for planes in [8usize, 16, 32, 64, 128] {
        let expected_bytes = planes.div_ceil(8);
