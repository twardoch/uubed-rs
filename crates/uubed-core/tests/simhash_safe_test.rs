...
use uubed_core::encoders::simhash_safe::{simhash, simhash_q64_safe};
...
fn test_simhash_safe_different_inputs() {
    // NB: SimHash is the sign of random-hyperplane projections, so it is
    // invariant under positive scaling. The inputs below are deliberately
    // non-collinear (no one is a positive scalar multiple of another); collinear
    // vectors such as [1,2,3,4,5] and [10,20,30,40,50] would hash identically.
    let embedding1 = vec![1, 2, 3, 4, 5];
    let embedding2 = vec![250, 10, 130, 40, 200];
    ...
    // Different (non-collinear) inputs should produce different hashes
...
fn test_simhash_safe_different_plane_counts() {
    ...
    let planes: [usize; 5] = [8, 16, 32, 64, 128];
    let mut hashes = Vec::new();

    for &plane_count in &planes {
        let hash = simhash(&embedding, plane_count);
        let expected_bytes = plane_count.div_ceil(8);
...
fn test_simhash_safe_thread_safety() {
    ...
    for _ in 0..20 {
        ...
        let handle = thread::spawn(move || simhash(&emb, 64));
        handles.push(handle);
    }
