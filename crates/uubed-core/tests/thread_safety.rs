use std::sync::Arc;
use std::thread;
use uubed_core::encoders::{q64_encode, simhash_q64, top_k_q64, z_order_q64};
...
fn test_concurrent_q64_encoding() {
    ...
    for _ in 0..100 {
        ...
        let handle = thread::spawn(move || q64_encode(&data));
        handles.push(handle);
    }
...
fn test_concurrent_simhash() {
    ...
    for _ in 0..50 {
        ...
        let handle = thread::spawn(move || simhash_q64(&emb, 64));
        handles.push(handle);
    }
...
fn test_concurrent_topk() {
    ...
    for _ in 0..20 {
        let emb = Arc::clone(&embedding);
        let handle = thread::spawn(move || top_k_q64(&emb, 16));
        handles.push(handle);
    }
...
fn test_concurrent_zorder() {
    ...
    for _ in 0..30 {
        let emb = Arc::clone(&embedding);
        let handle = thread::spawn(move || z_order_q64(&emb));
        handles.push(handle);
    }
