use uubed_core::encoders::q64::Q64Error;
use uubed_core::encoders::{mq64_decode, q64_decode};
use uubed_core::error::{UubedError, UubedResult};
...
fn test_q64_decode_odd_length_input() {
    let odd_length_inputs = ["a", "abc", "abcde", "abcdefg", "abcdefghi"];

    for input in &odd_length_inputs {
        let result = q64_decode(input);
        assert!(
            result.is_err(),
            "Should fail for odd length input: {}",
            input
        );
    }
}
...
fn test_ubbed_result_type() {
    ...
    let failure: UubedResult<Vec<u8>> = Err(UubedError::ValidationError(
        uubed_core::error::ValidationErrorKind::EmptyInput {
