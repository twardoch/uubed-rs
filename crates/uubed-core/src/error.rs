// this_file: crates/uubed-core/src/error.rs
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Q64ErrorKind::OddLength { length } => {
                write!(
                    f,
                    "Input has odd length {}, Q64 requires even length",
                    length
                )
            }
            Q64ErrorKind::InvalidCharacter {
                character,
                position,
            } => {
                write!(
            ...
            Q64ErrorKind::WrongPosition {
                ...
                write!(
                    f,
                    "Character '{}' at position {} belongs to alphabet {}, not expected alphabet",
                    character, position, expected_alphabet
                )
            }
            Q64ErrorKind::BufferOverflow {
                required,
                available,
            } => {
                write!(
                    f,
                    "Buffer overflow: need {} bytes, only {} available",
                    required, available
                )
            }
        }
...
impl fmt::Display for SimHashErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SimHashErrorKind::DimensionsTooLarge {
                dimensions,
                max_supported,
            } => {
                write!(
                    ...
                    dimensions, max_supported
                )
            }
            SimHashErrorKind::MatrixGenerationFailed { planes, dimensions } => {
                write!(
                    f,
                    "Failed to generate matrix for {} planes × {} dimensions",
                    planes, dimensions
                )
            }
...
impl fmt::Display for TopKErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TopKErrorKind::EmbeddingTooLarge {
                size,
                max_supported,
            } => {
                write!(
                    f,
                    "Embedding size {} exceeds maximum supported {}",
                    size, max_supported
                )
            }
...
impl fmt::Display for ValidationErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ValidationErrorKind::InputTooLarge {
                size,
                max_size,
                operation,
            } => {
                write!(
                    f,
                    "Input size {} exceeds maximum {} for operation: {}",
                    size, max_size, operation
                )
            }
...
pub mod validation {
    ...
    pub fn validate_embedding(embedding: &[u8], operation: &str) -> UubedResult<()> {
        if embedding.is_empty() {
            return Err(UubedError::ValidationError(
    ...
    pub fn validate_simhash_params(planes: usize, dimensions: usize) -> UubedResult<()> {
        if planes == 0 {
            return Err(UubedError::SimHashError(SimHashErrorKind::InvalidPlanes {
                planes,
            }));
        }
        ...
        if dimensions > MAX_SIMHASH_DIMENSIONS {
            return Err(UubedError::SimHashError(
                SimHashErrorKind::DimensionsTooLarge {
                    dimensions,
...
pub mod recovery {
    ...
    pub fn clamp_k_value(k: usize, embedding_size: usize) -> usize {
        k.min(embedding_size).clamp(1, validation::MAX_K_VALUE)
    }

    /// Clamp planes value to valid range
    pub fn clamp_planes_value(planes: usize) -> usize {
        planes.clamp(1, validation::MAX_SIMHASH_PLANES)
    }
}
