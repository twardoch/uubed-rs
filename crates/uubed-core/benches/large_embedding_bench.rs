...
use rand::prelude::*;
use uubed_core::encoders::{q64_encode, simhash_q64, top_k_q64, topk_optimized, z_order_q64};
...
fn generate_large_embedding(size: usize, pattern: &str) -> Vec<u8> {
    ...
    match pattern {
        ...
        "gradient" => {
            ...
            (0..size)
                .map(|i| ((i as f64 / size as f64) * 255.0) as u8)
                .collect()
        }
    ...
    for mb in [1, 4, 8, 16].iter() {
        ...
        group.bench_function(BenchmarkId::new("encode", format!("{}MB", mb)), |b| {
            b.iter(|| {
                ...
                black_box(q64_encode(&data[..chunk_size.min(data.len())]))
            })
        });
    }
        ...
        for pattern in &patterns {
            ...
            for &k in [64, 256, 1024].iter() {
                ...
                group.bench_function(
                    ...
                    |b| b.iter(|| black_box(top_k_q64(&embedding, k))),
                );
                ...
                group.bench_function(
                    ...
                    |b| b.iter(|| black_box(topk_optimized::top_k_q64_optimized(&embedding, k))),
                );
            }
        }
    }
    ...
    for &size in [1_000_000, 5_000_000, 10_000_000].iter() {
        ...
        for &planes in [64, 128, 256].iter() {
            ...
            group.bench_function(
                BenchmarkId::new(
                    "simhash",
                    format!("size={}_planes={}", size / 1_000_000, planes),
                ),
                |b| b.iter(|| black_box(simhash_q64(&embedding, planes))),
            );
        }
    }
    ...
    for &dims in [128, 256, 512, 1024].iter() {
        ...
        for pattern in ["random", "clustered"].iter() {
            ...
            group.bench_function(
                BenchmarkId::new(
                    "zorder",
                    format!("dims={}_count={}_pattern={}", dims, count, pattern),
                ),
                |b| {
                    b.iter(|| {
...
fn bench_scaling_analysis(c: &mut Criterion) {
    ...
    let sizes = vec![
        100_000, 500_000, 1_000_000, 5_000_000, 10_000_000, 20_000_000,
    ];
