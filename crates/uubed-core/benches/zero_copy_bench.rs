// this_file: crates/uubed-core/benches/zero_copy_bench.rs

use criterion::{black_box, criterion_group, criterion_main, BenchmarkId, Criterion};
use uubed_core::encoders::{q64_encode, q64_encode_to_buffer};
...
fn benchmark_zero_copy_vs_string(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        // Zero-copy version
        group.bench_with_input(BenchmarkId::new("zero_copy", size), &data, |b, data| {
            let mut buffer = vec![0u8; data.len() * 2];
            b.iter(|| {
                let _ = black_box(q64_encode_to_buffer(
                    black_box(data),
