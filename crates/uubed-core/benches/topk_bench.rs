...
use rand::prelude::*;
use uubed_core::encoders::{topk, topk_optimized};
        ...
        for k in [8, 16, 32, 64, 128].iter() {
            if *k <= *size {
                group.bench_with_input(
                    ...
                    &(&embedding, *k),
                    |b, (emb, k)| b.iter(|| topk::top_k_indices(black_box(emb), black_box(*k))),
                );
...
fn bench_sparse_embeddings(c: &mut Criterion) {
    ...
    for sparsity in [0.5, 0.8, 0.9, 0.95].iter() {
        ...
        group.bench_with_input(
            ...
            &(&embedding, k),
            |b, (emb, k)| {
                b.iter(|| topk_optimized::top_k_indices_optimized(black_box(emb), black_box(*k)))
            },
        );
    }
