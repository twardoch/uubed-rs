// This example `include!`s whole encoder source files to micro-benchmark a few
// internal functions, so most of the pulled-in items are intentionally unused.
#![allow(dead_code)]
...
fn benchmark_topk(
    ...
    println!(
        "{}: {} iterations in {:?} (avg: {} µs/op)",
        name, iterations, duration, avg_micros
    );
}
