use uubed_native::encoders::{mq64_decode, mq64_encode, mq64_encode_with_levels};
...
fn test_mq64_performance_regression() {
    ...
    assert!(
        encode_time.as_millis() < 1000,
        "Encoding took {} ms",
        encode_time.as_millis()
    );
    assert!(
        decode_time.as_millis() < 1000,
        "Decoding took {} ms",
